...
# df
# 
# write.csv(df, file = "exp1_raw.csv", row.names = FALSE, fileEncoding = "UTF-8")
# 
...
```{r}
# 读取原始数据
df <- read.csv("exp1_raw.csv", fileEncoding = "UTF-8")
...
```{r}
# 固定效应，条件间差异分布
df_rt_main_effect <- df_rt_pop %>%
  tidyr::pivot_wider(names_from = c(term), values_from = value) %>%
  dplyr::mutate(
    RT_Self = (Self_PSelf + Self_PFriend + Self_PStranger)/3,
    RT_Friend = (Friend_PSelf + Friend_PFriend + Friend_PStranger)/3,
    RT_Stranger = (Stranger_PSelf + Stranger_PFriend + Stranger_PStranger)/3,
  )%>%
  dplyr::select(`.chain`, `.iteration`, `.draw`,
    RT_Self, RT_Friend, RT_Stranger
  )
...
```{r}
bayestestR::sexit(df_rt_main_effect[,c(4:6)])
```
